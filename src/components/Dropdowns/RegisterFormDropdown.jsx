  ...
  return (
    <div>
      ...
      <div className={classNames("absolute transition duration-100 bg-white p-2 mt-1 w-[45vw] md:w-[16vw] rounded-xl shadow-2xl", {
            ...
            {options.map((option) => <button key={option.id} ><div className={classNames('text-gray-600 border-b-2 border-gray-200 text-start px-4 w-[42vw] md:w-[15vw] py-2 hover:bg-violet-500 hover:text-white rounded-md')}  onClick={()=> {setFromNo(option.id); setActive(false)}} >
              {option.label}
            </div></button>)}
        </div>
    </div>
