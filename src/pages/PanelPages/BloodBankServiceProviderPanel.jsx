...
function BloodBankServiceProviderPanel() {
    ...
    const collectProviderData = async () => {
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/bloodbanks/getparticularprovider/${RegdId.toUpperCase()}`);
    ...
    const handleDelete = async (regdId) => {
        try {
            console.log(regdId);
            const response = await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api//bloodbanks/${regdId}`);
            handleLogout();
        } catch (err) {
            console.log(err);
        }
    ...
    const getEnquiries = async () => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/getallrelatedenquiry`, data);
